...
CREATE OR REPLACE FUNCTION kiem_tra_mat_khau_moi(p_Mat_khau_hien_tai VARCHAR(60), p_Mat_khau_moi VARCHAR(60)) RETURNS BOOLEAN AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE them_nhan_vien(p_ho_va_ten VARCHAR(30), p_ngay_sinh DATE, p_gioi_tinh gioi_tinh, p_dia_chi VARCHAR(30), p_sdt VARCHAR(12), p_so_tk_ngan_hang VARCHAR(30), p_trinh_do_hoc_van VARCHAR(30), p_ten_tai_khoan VARCHAR(30), p_mat_khau_bam VARCHAR(60)) AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE sua_nhan_vien_qua_TK_va_MK(p_Ten_tai_khoan VARCHAR(30), p_Mat_khau VARCHAR(60), p_Ho_va_ten VARCHAR(30) = NULL, p_Ngay_sinh DATE = NULL, p_Gioi_tinh gioi_tinh = NULL, p_Dia_chi VARCHAR(30) = NULL, p_Sdt VARCHAR(12) = NULL, p_So_tk_ngan_hang VARCHAR(30) = NULL, p_Trinh_do_hoc_van VARCHAR(30) = NULL) AS $$
BEGIN
    -- Kiểm tra các ràng buộc dữ liệu
    IF p_ngay_sinh IS NOT NULL AND NOT kiem_tra_tuoi(p_ngay_sinh) THEN
        RAISE EXCEPTION 'Tuổi nhân viên phải lớn hơn 16 tuổi';
...
CREATE OR REPLACE PROCEDURE sua_nhan_vien_qua_ma_nhan_vien(p_Ma_nhan_vien UUID, p_Ho_va_ten VARCHAR(30) = NULL, p_Ngay_sinh DATE = NULL, p_Gioi_tinh gioi_tinh = NULL, p_Dia_chi VARCHAR(30) = NULL, p_Sdt VARCHAR(12) = NULL, p_So_tk_ngan_hang VARCHAR(30) = NULL, p_Trinh_do_hoc_van VARCHAR(30) = NULL, p_Ten_tai_khoan VARCHAR(30) = NULL, p_Mat_khau VARCHAR(60) = NULL) AS $$
BEGIN
    -- Kiểm tra các ràng buộc dữ liệu
    IF p_ngay_sinh IS NOT NULL AND NOT kiem_tra_tuoi(p_ngay_sinh) THEN
        RAISE EXCEPTION 'Tuổi nhân viên phải lớn hơn 16 tuổi';
...
CREATE OR REPLACE FUNCTION get_employee_data (role_list varchar[], gender_list varchar[], order_attributes varchar[] = ARRAY['gioi_tinh', 'ho_va_ten', 'vai_tro', 'ngay_vao_lam', 'ngay_sinh'], order_directions varchar[] = ARRAY['ASC', 'ASC', 'ASC', 'ASC', 'ASC'], isSelectAll BOOLEAN = TRUE, ma_cua_hang UUID = '12345678-1234-1234-1234-123456789012') RETURNS TABLE (ma_nhan_vien UUID,
...
CREATE OR REPLACE FUNCTION get_partial_name_data (partial_name text) RETURNS TABLE (ma_nhan_vien UUID,
                                                                                                 ten_nhan_vien varchar(255)) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ma_nhan_vien,
